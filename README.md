...
> **zsh 사용자 주의:** rustup은 PATH 설정을 `~/.profile`에 넣는데, zsh는 이 파일을 읽지 않습니다.
> 새 터미널에서 `cargo --version`이 안 되면 아래 한 줄을 `~/.zshrc`에 추가하세요.
>
> ```bash
> echo '. "$HOME/.cargo/env"' >> ~/.zshrc && source ~/.zshrc
> ```
>
> (이걸 안 하면 `npm run tauri dev` 실행 시 `cargo metadata ... No such file or directory` 에러가 납니다.)
